...
namespace srag\Plugins\UdfEditor\Libs\CustomInputGUIs\TextInputGUI;
...
class TextInputGUIWithModernAutoComplete extends TextInputGUI
{
    ...
    public static function init(?ilPlugin $plugin = null): void
    {
        if (self::$init === false) {
            ...
            $DIC->ui()->mainTemplate()->addJavaScript($version_parameter->appendToUrl($dir . "/../../node_modules/babel-polyfill/dist/polyfill.min.js"));

            $DIC->ui()->mainTemplate()->addJavaScript($version_parameter->appendToUrl(
                $dir . "/js/text_input_gui_with_modern_auto_complete.min.js",
                $dir . "/js/text_input_gui_with_modern_auto_complete.js"
            ));

            $DIC->ui()->mainTemplate()->addOnLoadCode("il.textinput_more_txt = " . json_encode($DIC->language()->txt('autocomplete_more')
                    . ";") . ";");
        }
    }
    ...
    public function render(string $a_mode = ""): string
    {
        ...
        /*if ($this->getDataSource()) {
            ...
            $tpl->setVariable('MORE_TXT', $this->dic->language()->txt('autocomplete_more'));
        }*/
        $tpl->setVariable('URL_AUTOCOMPLETE', $this->getDataSource());
